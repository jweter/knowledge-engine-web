...
def main() -> int:
    ...
    if github_repo and repo != github_repo:
        errors.append(f"repository {repo!r} does not match GITHUB_REPOSITORY {github_repo!r}")
    ...
    if not isinstance(verification, dict):
        ...
        auto = verification.get("automated_lanes")
        reality = verification.get("product_reality_lanes")
        executable = verification.get("executable_lanes")
        if not isinstance(auto, list) or not auto:
            errors.append("verification.automated_lanes must be non-empty")
        if not isinstance(reality, list) or not reality:
            errors.append("verification.product_reality_lanes must be non-empty")
