...
| Accessibility | 5.5 | `tests/test_accessibility_e2e.py` (added 2026-09-11, widened same day) now runs real axe-core checks against seven real pages -- the homepage, both Ask states, the claim detail page, the graph summary, the Evidence Intelligence dashboard, and the claims list -- enforced on every impact level axe-core reports (critical/serious/moderate/minor, not just critical/serious), wired into the same advisory `browser-e2e.yml` CI job. All seven pages verified at zero violations at any impact level. This is real automated evidence, not an assumed pass -- but axe-core's automated ruleset only catches roughly 30-50% of WCAG 2.2 AA success criteria industry-wide, no manual keyboard/screen-reader pass has been performed, and the async-Research progress/report views and the mobile review panel remain unchecked (both require real Research capability to exercise honestly). Do not treat this as WCAG 2.2 AA conformance evidence. |
...
`tests/test_accessibility_e2e.py` (added 2026-09-11, widened same day) now runs real axe-core checks against seven real pages -- the homepage, both Ask states, the claim detail page, the graph summary, the Evidence Intelligence dashboard, and the claims list -- on every PR, enforced on every impact level (critical/serious/moderate/minor), and all seven currently pass with zero violations at any impact level. That is real, verified automated evidence where none existed before -- but it is not WCAG 2.2 AA conformance evidence: axe-core's automated ruleset only catches roughly 30-50% of success criteria industry-wide, no manual keyboard-navigation/screen-reader pass has been performed, and the async-Research progress/report views and mobile review panel remain unchecked because exercising them honestly requires real Research capability. Perform an actual manual keyboard/screen-reader pass, and extend axe coverage to those two remaining surfaces once Research capability is verifiable, before calling this gap fully closed.
