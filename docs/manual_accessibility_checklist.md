...
| 9 | 2.4.6 Headings and Labels | `tests/test_heading_landmark_structure_e2e.py` automates the objective part: no heading level is skipped (an h2 followed directly by an h4, for example) and every page has exactly one top-level h1, across all 18 pages/states `tests/test_accessibility_e2e.py` covers; and exactly one `main` landmark exists, with any repeated landmark role (e.g. two `nav` regions) carrying its own non-empty, distinct accessible name so a screen reader's landmark list does not show indistinguishable entries. `axe-core`'s `heading-order`/`landmark-one-main`/`landmark-unique` rules are tagged "best practice" and are not part of its default enabled ruleset, so nothing previously caught this. All 18 pages currently pass. What remains manual: whether a screen reader user actually *understands* a given heading or landmark's meaning out of context -- a subjective judgment this check cannot make | All 18 pages/states `test_accessibility_e2e.py` covers |
