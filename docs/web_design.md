...
`knowledge-engine-ai`'s `SessionRepository` already durably persists a `ResearchSession` header plus one `ResearchEvent` row per completed workflow step to `Settings.session_db_path` as `/ask?synthesize=1` executes -- a real SQLite file already covered by the existing `session_storage_mode`/`session_persistent_root` local/persistent split, so it already survives a Render redeploy. Rather than building a second, Web-owned session store next to that one, this slice adds a strictly read-only projection of it: `knowledge_engine_web/research_session_status.py` opens the store `mode=ro` and derives a small status view (session id, question, status, a `current_stage` label mapped from the latest durably recorded `workflow_node` using the same stage vocabulary `docs/general_question_research_loop_v1.md`'s "Recommended visitor-facing progression" already documents, `terminal`, timestamps, event count). A new `GET /ask/session/{session_id}` route exposes that view as JSON; an unknown session id or a store that does not exist yet both 404. `/ask`'s own synchronous request/response shape is unchanged -- this route is additive. Live-verified against a real `uvicorn` process: seeded a session through the real `SessionRepository`, read it back over HTTP, then started an entirely new server process against the same on-disk file to confirm the state survives a process restart. Not yet done: no frontend polls this endpoint, and `/ask?synthesize=1` still runs the whole research session inline within one HTTP request rather than as a background task, so there is nothing to poll *during* an in-flight run yet -- only after it finishes, or after a redeploy for an already-recorded session. See `docs/general_question_research_loop_v1.md` WEB-GQR-4.
